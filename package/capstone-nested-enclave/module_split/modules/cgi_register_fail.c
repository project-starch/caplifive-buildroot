...
void cgi_register_fail(char *socket_fd_region_base) {
	char name[16];
    ...
    sprintf(content,"%s<title>Registration failure</title>\n",content);
    sprintf(content,"%s</head>\n",content);
    sprintf(content,"%s<body>\n",content);
    sprintf(content,"%s<p>Hello, %s. Something went wrong.<\\p>\n",content,name);
...
EXPORT_SYMBOL(cgi_register_fail);

static int __init cgi_register_fail_init(void)
{
	printk(KERN_INFO "Entering CGI Register Fail module.");

	return 0;
}

static void __exit cgi_register_fail_exit(void)
{
	printk(KERN_INFO "Exiting CGI Register Fail module.");
}

module_init(cgi_register_fail_init);
module_exit(cgi_register_fail_exit);
