...
define CAPSTONE_NULL_BLK_BUILD_CMDS
	...
	$(MAKE) -C '$(@D)'/driver LINUX_DIR='$(LINUX_DIR)' PWD='$(@D)'/driver CC='$(TARGET_CC)' LD='$(TARGET_LD)' \
		KERNEL_ARCH=$(KERNEL_ARCH) TARGET_CROSS=$(TARGET_CROSS)
	$(MAKE) -C '$(@D)'/split_driver LINUX_DIR='$(LINUX_DIR)' PWD='$(@D)'/split_driver CC='$(TARGET_CC)' LD='$(TARGET_LD)' \
		KERNEL_ARCH=$(KERNEL_ARCH) TARGET_CROSS=$(TARGET_CROSS)
endef
...
define CAPSTONE_NULL_BLK_INSTALL_TARGET_CMDS
	...
	$(INSTALL) -D -m 0755 '$(@D)/driver/null_blk.ko' '$(TARGET_DIR)/nullb/module_split/null_blk.ko'
	$(INSTALL) -D -m 0755 '$(@D)/split_driver/nullb_split.ko' '$(TARGET_DIR)/nullb/module_split/nullb_split.ko'
endef
