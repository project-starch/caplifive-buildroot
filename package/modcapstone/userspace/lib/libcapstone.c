...
void shared_region_annotated(dom_id_t dom_id, region_id_t region_id, unsigned long annotation_perm, unsigned long annotation_rev) {
    ...
    debug_counter_tick(DEBUG_COUNTER_SWITCH_U);
    /* The monitor reports region_id/perm/rev all arriving as ZERO here while dom_id
       arrives correctly, and every link in the C chain checks out (this call site, the
       Linux SBI arg->register mapping, the monitor's per-register RESTORE_REG offsets,
       the 8-param dispatch). DOM_CREATE moves four arguments through the same path fine.
       This print is the one link never instrumented: it proves whether the struct handed
       to the ioctl is populated at all. If it shows 12/1/2 the loss is in
       copy_from_user or the SBI transition; if it shows zeros the bug is here in
       libcapstone and never involved the monitor. printf is used because it demonstrably
       reaches the console on this board (the loader's own output does). */
    printf("LC: rgid=%lu prm=%lu rev=%lu dom=%lu\n",
           (unsigned long)args.region_id, (unsigned long)args.annotation_perm,
           (unsigned long)args.annotation_rev, (unsigned long)args.dom_id);
